// app/(admin)/admin/quotes/[id]/page.js
"use client";
import PageLink from "@/components/PageLink";
import { handleQuoteImageDelete } from "@/utils/deleteImageHelper";
    ...
    const fetchQuote = async () => {
      try {
        ...
        if (!res.ok || !quote.success) {
          throw new Error(quote.error || "Failed to fetch quote");
        }
        setFormData({
          ...quote.quote,
          image: quote.quote.image
            ? { url: quote.quote.image, preview: quote.quote.image }
            : null,
          active: quote.quote.active || false,
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.image?.file) {
        form.append("image", formData.image.file); // new upload
      }
            ...
            <div className="w-full flex flex-col gap-2.5">
              ...
              {formData.image?.preview && (
                <div
                  ...
                  style={{
                    backgroundImage: `url(${formData.image.preview})`,
                  }}
                >
                  <button
                    type="button"
                    onClick={() =>
                      handleQuoteImageDelete(
                        quoteId,
